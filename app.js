...
(() => {
  ...
  function drawBackground(){
    ...
    // far city
    ctx.globalAlpha = 0.18;
    ctx.fillStyle = '#274b6b';
    for (let i=0;i<18;i++){
      const bx = i*60 + ((world.t*0.4)%60);
      const bw = 40;
      const bh = 30 + (i%5)*16;
      ctx.fillRect((bx% (W+60))-60, world.groundY-200-bh, bw, bh);
    }
    ctx.globalAlpha = 1;
  ...
  function checkAnswer(){
    ...
    const userAns = normalizeNumber(answerInput.value);
    if (!Number.isFinite(userAns)){
      qFeedback.textContent = 'اكتب رقمًا صحيحًا.';
    ...
    if (userAns === currentQuestion.answer){
      ...
      qFeedback.textContent = `❌ إجابة غير صحيحة. جرّب مرة أخرى.`;
      qFeedback.className = 'feedback bad';
  ...
  function revive(){
    // reset pipes & bird but keep score (so feels like continue)
    bird.reset();
    pipes.reset();
    particles.length = 0;
    STATE.awaitingQuestion = false;
